...
uses
  ...
  ComCtrls, Spin, ACBrSocket, ACBrCNIEE;
...
uses
  TypInfo, uFuncoes, uAutocomConsts;

{$R *.dfm}

procedure Tfrmaac.ACBrAAC1GetChave(var Chave: AnsiString);
begin
  Chave := #22 + #24 + #13 + #234;
end;

procedure Tfrmaac.AdicionarItemGrid(const ANumeroSerie: String;
  const AValorGT: Double; const ACRO: Integer; const ACNIEE: Integer);
begin
...
function Tfrmaac.StrToNumero(const AString: String): Double;
var
  StrTmp: String;
begin
...
begin
   ...
   if cniee.DownloadTabela then
      lbl1.Caption := 'Download concluido...';

   CNIEE.AbrirTabela;

   for I := 0 to Pred(CNIEE.Cadastros.Count) do
   begin
     edCNIEE.Items.Append(CNIEE.Cadastros[I].DescrMarca + '   ' +
                          CNIEE.Cadastros[I].DescrModelo + '   ' +
                          CNIEE.Cadastros[I].Versao);
   end;
end;

procedure Tfrmaac.FormCreate(Sender: TObject);
var
...
procedure Tfrmaac.edGTECFKeyPress(Sender: TObject; var Key: Char);
begin
  if not( Key in ['0'..'9', ',', #8] ) then
    Key := #0
end;

procedure Tfrmaac.recriarArquivoAuxiliar;
var
  F: TextFile;
  ct: integer;
begin
  try
    ...
    ACBrAAC1.IdentPAF.NumeroLaudo            := 'POL3582013';
    ACBrAAC1.IdentPAF.VersaoER               := '01.02';

    ACBrAAC1.IdentPAF.Empresa.RazaoSocial    := 'CLAUDIA LUCIA GURGEL BARBOSA';
    ACBrAAC1.IdentPAF.Empresa.CNPJ           := '05213425000100';
    ACBrAAC1.IdentPAF.Empresa.IE             := 'ISENTO';
    ACBrAAC1.IdentPAF.Empresa.IM             := '3244';
    ACBrAAC1.IdentPAF.Empresa.Endereco       := 'Rua Juscelino Kubitschek, 446';
    ACBrAAC1.IdentPAF.Empresa.Cep            := '38880000';
    ACBrAAC1.IdentPAF.Empresa.Cidade         := 'Tiros';
    ACBrAAC1.IdentPAF.Empresa.Email          := 'sac@paradigmati.com.br';
    ACBrAAC1.IdentPAF.Empresa.Contato        := 'Alfredo Koplin';
    ACBrAAC1.IdentPAF.Empresa.Uf             := 'MG';
    ACBrAAC1.IdentPAF.Empresa.Telefone       := '3436715423';
...
procedure Tfrmaac.buscaInformacoesArquivoAuxiliar;
var
  ct : integer;
begin
  ACBrAAC1.NomeArquivoAux := GetPathArquivoAuxiliar;
...
procedure Tfrmaac.btnECFCapturarDadosClick(Sender: TObject);
begin
...
procedure Tfrmaac.btnAdicionarECFClick(Sender: TObject);
var
   s: string;
begin
   if edCNIEE.ItemIndex>=0 then
   begin

   s := CNIEE.Cadastros[edCNIEE.ItemIndex].CodMarca +
        CNIEE.Cadastros[edCNIEE.ItemIndex].CodModelo +
        CNIEE.Cadastros[edCNIEE.ItemIndex].CodVersao;
   end
   else
      s :='000000';
  ...
  try
    ...
    AdicionarItemGrid(edNumeroSerieECF.Text, StrToNumero(edGTECF.Text), edCROECF.Value, StrToInt(s));
  except
    on E: Exception do
    begin
      edNumeroSerieECF.SetFocus;
...
procedure Tfrmaac.btnExcluirECFsClick(Sender: TObject);
begin
  lstECFsAutorizados.Clear;
end;

procedure Tfrmaac.btnSalvarArquivoClick(Sender: TObject);
begin
  recriarArquivoAuxiliar;
end;

procedure Tfrmaac.btnSairAplicativoClick(Sender: TObject);
begin
  close;
end;
