...
Não tributado	      41	            400	   N  devolu, trf, etc, incusive Serviços (municipal)
ICMS ST já cobrado	60	            500	   F
...
begin
   ...
   if DM_INI.INI.StoredValue['modo_farmacia'] then //acrescenta as opções no cmbOpPesq
   begin
   ...
   begin
      if (DM_INI.INI.StoredValue['modo_outros'])and(DM_INI.INI.StoredValue['prefix_cod_balanca']>0) then
         CmbOpPesq.Properties.Items.Append('SOMENTE PROD. BALANÇA (7)');
...
begin
  ...
  if TabCad.Visible then
     cmbSessao.SetFocus;
     //editando é true se vem da entrada de nf
...
begin
   ...
   begin
      ...
      begin
          ...
          begin
             cmbSessao.SetFocus;
             raise Exception.Create('Informe a sessão.');
          end;

          if (DM.QEstoquedestinacao.AsString = '') then
          begin
             cmbDestinacao.SetFocus;
             raise Exception.Create('Informe a destinação.');
          end;
    ...
    {  //verifica se o cod ja existe
      ...
      begin
         ShowMessage('Já existe um produto com este código GTIN.');
